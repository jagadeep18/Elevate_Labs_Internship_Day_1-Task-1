...
3. **Determine subnet range** (e.g., for `172.168.10.73` with subnet mask `255.255.224.0`, CIDR = `/19`)
...
8. **Export results** to a file:
   ...
   ```
   nmap -sS 172.168.10.73 -oN scan_results.txt
   ```
...
### Target IP: `172.168.10.73`

| Port | State | Service      | Risk Level | Notes                      |
| ---- | ----- | ------------ | ---------- | -------------------------- |
| 135  | open  | msrpc        | High       | RPC vulnerable to RCE      |
| 139  | open  | netbios-ssn  | High       | NetBIOS file sharing       |
| 445  | open  | microsoft-ds | Critical   | SMB exploits (EternalBlue) |
| 5357 | open  | wsdapi       | Medium     | Device exposure risk       |
| 8000 | open  | http-alt     | Medium     | Dev server/dashboard       |
| 8089 | open  | unknown      | Unknown    | Possible custom app        |
